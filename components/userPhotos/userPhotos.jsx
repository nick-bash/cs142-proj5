...
class UserPhotos extends React.Component {
  constructor(props) {
    ...
    var promise = FetchModel("http://localhost:3000/photosOfUser/" + this.props.match.params.userId);
    promise.then(response => {
        this.setState({photos: response.data});
      })
      .catch(err => console.err(err));
      
    promise = FetchModel("http://localhost:3000/user/" + this.props.match.params.userId);
    promise.then(response => {
